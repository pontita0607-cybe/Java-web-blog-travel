...
@Controller
public class ZootypeController {
	
	@Autowired
    BlogRepository repo;
    
    @GetMapping("/zooType")
    public String showZooBlogs(Model model, HttpSession session) {
        // ดึงข้อมูลบล็อกที่มีประเภทเป็น "ทะเล" จากฐานข้อมูล
    	if (session.getAttribute("user") != null) {
    		User user = (User) session.getAttribute("user");
            model.addAttribute("name", user.getName());
            model.addAttribute("email", user.getEmail());
        List<Blog> zooBlogs = repo.findByType("สวนสัตว์");
        
        // ส่งข้อมูลไปยัง seaType.jsp
        model.addAttribute("zooBlogs", zooBlogs);
